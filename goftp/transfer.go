...
// TransferRangeUpload 这是魔改的函数，服务器不支持upload的断点续传
// 所以我们把上传位置改为0, 保证上传数据量，不保证文件正确错误
func (c *Client) TransferRangeUpload(path string, dest io.Writer, src io.Reader, offset, len int64) (int64, error) {
    ...
    if offset > 0 {
        err := pconn.sendCommandExpected(replyFileActionPending, "REST %d", 0)
        if err != nil {
            return 0, err
        }
    }
    ...
    if err == nil {
        _ = pconn.sendCommandExpected(226, "ABOR")
        code, msg, err := pconn.readResponse() // 426
        if code != 426 && err == nil {
            err = ftpError{code: code, msg: msg}
        }
        if err == nil {
            if code, msg, err = pconn.readResponse(); code != 450 && err == nil {
                err = ftpError{code: code, msg: msg}
            }
        }
        _, _, _ = code, msg, err
...
// TransferRangeDownload ... 和upload 的行为有些不同，所以复制了一份代码
func (c *Client) TransferRangeDownload(path string, dest io.Writer, src io.Reader, offset, len int64) (int64, error) {
    pconn, err := c.getIdleConn()
    if err != nil {
        return 0, err
    }
    ...
    if err == nil {
        code, _, err := pconn.sendCommand("ABOR")

        if err != nil || (code != replyClosingDataConnection && code != replyLocalError) {
            fmt.Println(err)
            pconn.broken = true
            return n, nil
        }
    ...
    if !positiveCompletionReply(code) && code != replyCommandSyntaxError {
        pconn.debug("unexpected response after %s: %d (%s)", cmd, code, msg)
        return n, ftpError{code: code, msg: msg}
    }
